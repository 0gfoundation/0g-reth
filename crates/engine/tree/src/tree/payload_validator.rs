    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        // Send post-execution state to the multiproof task (includes withdrawals and other
        // post-block state changes that were not captured by the transaction state hook)
        handle.send_post_execution_state(&output.state);
        ...
        info!(
            "[Debug] Validate block with state, block={:?}, output={:?}, hashed_state={:?}",
            &block, &output, &hashed_state
        );
