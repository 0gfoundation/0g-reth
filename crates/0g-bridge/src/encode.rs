//! ABI encoding of `Bridge.parkRemoteMessages(InboundMessage[])` calldata.
//!
...
sol! {
    ...
    /// `Bridge.parkRemoteMessages(InboundMessage[])` — only callable from `SYSTEM_ADDRESS`.
    function parkRemoteMessages(InboundMessageAbi[] msgs) external;
}
...
/// Builds the full `Bridge.parkRemoteMessages([...])` ABI calldata payload.
///
...
pub fn encode_park_remote_messages_calldata(
    msgs: &[BridgeMessage],
    local_chain_id: u64,
    ...
    let call = parkRemoteMessagesCall { msgs: abi_msgs };
...
mod tests {
    ...
    fn empty_input_produces_empty_array_calldata() {
        let cd = encode_park_remote_messages_calldata(&[], 16700, FEE_RECIPIENT);
        // Should still be a valid call (selector + empty array).
        assert!(cd.len() >= 4, "must contain selector");
        let decoded = parkRemoteMessagesCall::abi_decode(&cd).expect("roundtrip empty");
    ...
    fn dst_chain_filter_drops_mismatched_messages() {
        ...
        let cd = encode_park_remote_messages_calldata(&msgs, 16700, FEE_RECIPIENT);
        let decoded = parkRemoteMessagesCall::abi_decode(&cd).expect("roundtrip");
    ...
    fn abi_roundtrip_preserves_all_carried_fields() {
        ...
        let cd = encode_park_remote_messages_calldata(&[m.clone()], 16702, FEE_RECIPIENT);
        let decoded = parkRemoteMessagesCall::abi_decode(&cd).expect("roundtrip");
    ...
    #[test]
    fn calldata_starts_with_park_remote_messages_selector() {
        let m = msg_for(1, 16700, 1);
        let cd = encode_park_remote_messages_calldata(&[m], 16700, FEE_RECIPIENT);
        // Hardcoded keccak256("parkRemoteMessages((uint64,uint64,address,address,uint256,
        // address)[])")[..4] rather than `parkRemoteMessagesCall::SELECTOR` — comparing
        // against the macro's own output would let a typo in the `sol!` signature
        // self-consistently pass.
        let expected_selector = [0x7c, 0x31, 0xc3, 0x0f];
    ...
    fn abi_field_order_matches_solidity() {
        ...
        let cd = encode_park_remote_messages_calldata(&[m], 16700, FEE_RECIPIENT);
        let decoded = parkRemoteMessagesCall::abi_decode(&cd).unwrap();
    ...
    fn fee_recipient_is_stamped_on_every_message() {
        ...
        let cd = encode_park_remote_messages_calldata(&msgs, 16700, addr);
        let decoded = parkRemoteMessagesCall::abi_decode(&cd).expect("roundtrip");
