...
/// CL hardcodes the same value at `chain.BridgeContractAddressHex`. Surfaced through
/// `bridge_contract_address` only on 0G chains (gated by `bridge_activation_time > 0`); Ethereum L1
/// specs keep the activation time at 0 and report `None`, preserving upstream behaviour.
...
pub struct ChainSpec<H: BlockHeader = Header> {
    ...
    /// Unix timestamp at which the 0G bridge fork activates. Resolved from an explicit
    /// `bridgeForkTime` in the genesis JSON when present (devnet/custom chains), otherwise
    /// from the release-baked per-chain-id constant (`crate::bridge_activation_time` —
    /// mainnet/testnet scheduling ships as a binary upgrade, like staking).
...
impl From<Genesis> for ChainSpec {
    fn from(genesis: Genesis) -> Self {
        ...
        // 0G bridge fork — resolve the activation time. An explicit `bridgeForkTime` in the
        // genesis JSON `config` wins (devnet/custom chains, whose genesis is generated by the
        // CL's specgen); absent that, fall back to the release-baked per-chain-id constant
        // (`crate::bridge_activation_time`). Mainnet/testnet activation therefore ships as a
        // binary upgrade bumping the constant, same release model as staking — validators do
        // not edit their genesis files. The key rides in `extra_fields` rather than a typed
        // `genesis.config` field because the upstream alloy ChainConfig has no field for
        // 0G-custom forks (the bridge is the only 0G fork that reaches the EL;
        // rssymbiotic/slashing/etc. are CL-only). The proxy address itself is a compile-time
        // constant (`BRIDGE_PROXY_ADDRESS`) returned by the trait impl whenever activation
        // time is non-zero; we no longer plumb it through genesis.
        //
        // Parse it exactly like alloy parses the standard `*Time` fields (deserialize_u64_opt →
        // quantity): accept a decimal number OR a 0x-hex string (U64 handles both). But a
        // *present-but-malformed* value must fail loudly, NOT be swallowed into the fallback —
        // otherwise a stringified/typo'd timestamp boots the node with the bridge off and then
        // drops it out of consensus at the CL's fork boundary (V4 FCU / post-fork blocks
        // rejected) with no startup diagnostic.
        let bridge_activation_time =
            match genesis.config.extra_fields.get_deserialized::<U64>("bridgeForkTime") {
                None => crate::bridge_activation_time(genesis.config.chain_id),
                Some(Ok(t)) => t.to::<u64>(),
                Some(Err(err)) => panic!(
...
impl ChainSpecBuilder {
    ...
    pub fn mainnet() -> Self {
        Self {
            ...
            bridge_activation_time: MAINNET.bridge_activation_time,
        }
    }
}
...
impl ChainSpecBuilder {
    ...
    /// Enable the 0G Bridge fork at the given timestamp.
    pub const fn with_bridge_at(mut self, timestamp: u64) -> Self {
        self.bridge_activation_time = timestamp;
        self
    }
    ...
    pub fn build(self) -> ChainSpec {
        ...
        ChainSpec {
            ...
            bridge_activation_time: self.bridge_activation_time,
            ..Default::default()
        }
    }
}
...
impl From<&Arc<ChainSpec>> for ChainSpecBuilder {
    fn from(value: &Arc<ChainSpec>) -> Self {
        Self {
            ...
            bridge_activation_time: value.bridge_activation_time,
        }
    }
}
...
- Frontier                         @0"
    ...
    /// Without an explicit `bridgeForkTime`, the activation time falls back to the
    /// release-baked per-chain-id constant — mainnet/testnet bridge scheduling ships as a
    /// binary upgrade (like staking), so validators never edit their genesis files. Unknown
    /// chain IDs fall back to 0 (dormant). `pragueTime: 0` keeps the Prague-ordering
    /// invariant satisfied for any future non-zero constant.
    #[test]
    fn bridge_fork_time_falls_back_to_chain_id_constant() {
        for (chain_id, expected) in [
            (crate::ZG_MAINNET_CHAIN_ID, crate::BRIDGE_ACTIVATION_TIME_MAINNET),
            (crate::ZG_TESTNET_CHAIN_ID, crate::BRIDGE_ACTIVATION_TIME_TESTNET),
            (1337, 0),
        ] {
            let s = format!(
                r#"{{"config":{{"chainId":{chain_id},"shanghaiTime":0,"cancunTime":0,"pragueTime":0}},"nonce":"0x0","timestamp":"0x0","extraData":"0x","gasLimit":"0x4c4b40","difficulty":"0x1","alloc":{{}}}}"#
            );
            let genesis: Genesis = serde_json::from_str(&s).unwrap();
            let spec = ChainSpec::from(genesis);
            assert_eq!(spec.bridge_activation_time, expected, "chain id {chain_id}");
        }
    }

    /// An explicit `bridgeForkTime` in genesis overrides the per-chain-id constant, even on a
    /// chain ID that has a baked entry — devnet/custom chains schedule via specgen-generated
    /// genesis, and that explicit choice must win.
    #[test]
    fn bridge_fork_time_genesis_key_overrides_constant() {
        let s = format!(
            r#"{{"config":{{"chainId":{},"shanghaiTime":0,"cancunTime":0,"pragueTime":100,"bridgeForkTime":100}},"nonce":"0x0","timestamp":"0x0","extraData":"0x","gasLimit":"0x4c4b40","difficulty":"0x1","alloc":{{}}}}"#,
            crate::ZG_MAINNET_CHAIN_ID
        );
        let genesis: Genesis = serde_json::from_str(&s).unwrap();
        let spec = ChainSpec::from(genesis);
        assert_eq!(spec.bridge_activation_time, 100);
    ...
    #[test]
    fn bridge_activation_survives_builder_and_header_mapping_roundtrip() {
        let spec = ChainSpecBuilder::default()
            ...
            .build();
        assert_eq!(spec.bridge_activation_time, 100);
        assert!(!spec.is_bridge_active_at_timestamp(99));
        assert!(spec.is_bridge_active_at_timestamp(100));

        let rebuilt = ChainSpecBuilder::from(&Arc::new(spec.clone())).build();
        assert_eq!(rebuilt.bridge_activation_time, 100);

        let mapped = rebuilt.map_header(|header| header);
        assert_eq!(mapped.bridge_activation_time, 100);
        assert!(!mapped.is_bridge_active_at_timestamp(99));
        assert!(mapped.is_bridge_active_at_timestamp(100));
    }
