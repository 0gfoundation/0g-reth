...
mod bridge_tests {
    ...
    fn database_with_bridge_stub() -> CacheDB<EmptyDB> {
        ...
        db.insert_account_info(
            ...
            AccountInfo {
                ...
                code: Some(code),
                // reth-v2.4.1 migration: the revm version behind v2.4.1 adds `account_id` to
                // `AccountInfo`; 0G's fixture predates it.
                account_id: None,
            },
        );
        db
    ...
    fn run_with_bridge_ctx(spec: Arc<ChainSpec>, bridge_calldata: Option<Bytes>) -> (U256, U256) {
        ...
        // reth-v2.4.1 migration: three unrelated upstream changes land on this fixture.
        // `evm_for_block` became fallible (0G called it bare); `EthBlockExecutionCtx` gained
        // `tx_count_hint`, `extra_data` and `slot_number`; and `withdrawals` now holds a plain
        // `Vec` where 0G wrapped it in `Withdrawals::new(..)` (hence the dropped import).
        let evm = provider.evm_for_block(&mut state, &header).expect("valid EVM environment");
        let ctx = EthBlockExecutionCtx {
            tx_count_hint: None,
            parent_hash: header.parent_hash,
            parent_beacon_block_root: header.parent_beacon_block_root,
            ommers: &[],
            withdrawals: Some(Cow::Owned(vec![])),
            slashed: None,
    ...
    fn finish_requests_with_raw(
        ...
        let db = database_with_bridge_stub();
        let mut state = State::builder().with_database(db).with_bundle_update().build();
        // reth-v2.4.1 migration: same fallible `evm_for_block` and same new/retyped
        // `EthBlockExecutionCtx` fields as in `run_with_bridge_ctx` above.
        let evm = provider.evm_for_block(&mut state, &header).expect("valid EVM environment");
        let ctx = EthBlockExecutionCtx {
            tx_count_hint: None,
            parent_hash: header.parent_hash,
    ...
    fn bridge_call_uses_attributes_suggested_fee_recipient_in_build_path() {
        ...
        // reth-v2.4.1 migration: `ConfigureEvm`'s `context_for_*` methods returned the context
        // directly for 0G. v2.4.1 made them return `Result<_, Self::Error>`; `EthEvmConfig`
        // keeps `Error = Infallible`, so unwrapping here cannot panic.
        let ctx = provider.context_for_next_block(&parent, attrs).unwrap();
    ...
    fn bridge_call_uses_payload_beneficiary_in_verify_path_byte_equal_to_build() {
        ...
        // reth-v2.4.1 migration: fallible `context_for_*`, `Error = Infallible` (see above).
        let ctx_verify = provider.context_for_payload(&exec_data).unwrap();
    ...
    fn context_for_block_without_body_blob_has_no_bridge_ctx() {
        ...
        // reth-v2.4.1 migration: fallible `context_for_*`, `Error = Infallible` (see above).
        let ctx = provider.context_for_block(&sealed).unwrap();
        assert!(
