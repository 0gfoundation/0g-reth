...
async fn testing_rpc_build_block_works() -> eyre::Result<()> {
    ...
    let builder = NodeBuilder::new(config)
        ...
        .on_rpc_started(move |ctx, handles| {
            ...
            // reth-v2.4.1 migration: 0G used `EthPayloadAttributes` as a flat struct literal
            // here. It is now a wrapper (`inner` + `bridge_requests`), so the flat literal is
            // built from the standard `PayloadAttributes` that `inner` holds.
...
async fn testing_rpc_commit_block_works() -> eyre::Result<()> {
    ...
    let builder = NodeBuilder::new(config)
        ...
        .on_rpc_started(move |ctx, handles| {
            ...
            // reth-v2.4.1 migration: same wrapper change as in the test above.
            let payload_attributes = PayloadAttributes {
                timestamp,
                prev_randao: B256::ZERO,
                suggested_fee_recipient: Address::ZERO,
